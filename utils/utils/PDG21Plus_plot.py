...
def plot_mass_spectrum(input_df):
    df = input_df.copy()
...
def plot_mass_spectra_comparison(particle_lists):
    PDG21Plus_df = particle_lists[0].copy()
    PDG16Plus_df = particle_lists[1].copy()
    SMASH_df     = particle_lists[2].copy()
    ...
    SMASH_masses_nucleons = SMASH_df[SMASH_df['Name'].str.startswith('N')]['Mass(GeV)']
    ...
    PDG16Plus_masses_deltas = PDG16Plus_df[(PDG16Plus_df['Baryon no.']!=0) &\
                                 ...
                                 (PDG16Plus_df['Strangeness no.']==0)]['Mass(GeV)']
    SMASH_masses_deltas = SMASH_df[SMASH_df['Name'].str.startswith('Δ')]['Mass(GeV)']
    ...
    SMASH_masses_kaons = SMASH_df[SMASH_df['Name'].str.startswith('K')]['Mass(GeV)']
    ...
    SMASH_masses_lambdas = SMASH_df[SMASH_df['Name'].str.startswith('Λ')]['Mass(GeV)']
    ...
    SMASH_masses_xis = SMASH_df[SMASH_df['Name'].str.startswith('Ξ')]['Mass(GeV)']
    ...
    PDG16Plus_masses_sigmas = PDG16Plus_df[(PDG16Plus_df['Baryon no.']!=0) &\
                                 ...
                                 (PDG16Plus_df['Strangeness no.']==-1)]['Mass(GeV)']
    SMASH_masses_sigmas = SMASH_df[SMASH_df['Name'].str.startswith('Σ')]['Mass(GeV)']
    ...
    SMASH_masses_omegas = SMASH_df[SMASH_df['Name'].str.startswith('Ω')]['Mass(GeV)']
    ...
    output_path = os.path.join(parent_path, 'plots/')
    os.makedirs(output_path, exist_ok=True)
    fname = os.path.join(output_path, 'Fig_mass_spectra_comparison.pdf')
    plt.savefig(fname=fname, format='pdf')
    #plt.show()