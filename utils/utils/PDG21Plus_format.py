...
PDG21Plus_massorder_particles_filename = 'PDG21Plus_massorder.dat'
PDG21Plus_massorder_decays_filename = 'decays_PDG21Plus_massorder.dat'
PDG21Plus_ThFIST_particles_filename = 'PDG21Plus_ThFIST.dat'
...
def massorder_particles_format(input_df):
...
def massorder_full_decays_format(input_df):
    """Returns list ordered in descending mass order"""
    df = input_df.copy()
...
def ThFIST_particles_format(input_df):
...
def ThFIST_full_decays_format(input_df):
    """Returns list formatted intended to be used in Thermal FIST"""
    df = input_df.copy()
    df = df[df['ID']>=0]
    mask = df['Name'].str.isalpha().notnull()
...
def save_PDG21Plus_massorder_particles_df(df, output_path, header=False):
    if header!=True: header = None 
    path = os.path.join(output_path,PDG21Plus_massorder_particles_filename)
    df.to_csv(path, sep='\t', index=False, header=header)

def save_PDG21Plus_massorder_decays_df(df, output_path, header=False):
    if header!=True: header = None 
    path = os.path.join(output_path,PDG21Plus_massorder_decays_filename)
    df.to_csv(path, sep='\t', index=False, header=header)

def save_PDG21Plus_ThFIST_particles_df(df, output_path, header=False):
    if header!=True: header = None 
    path = os.path.join(output_path,PDG21Plus_ThFIST_particles_filename)
    df.to_csv(path, sep='\t', index=False, header=header)

def save_PDG21Plus_ThFIST_decays_df(df, output_path, header=False):
    if header!=True: header = None 
    path = os.path.join(output_path,PDG21Plus_ThFIST_decays_filename)
    f = open(path,'a')
