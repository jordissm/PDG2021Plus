...
def plot_mass_spectra_old_vs_new(old_input_df, input_df):
    old_df = old_input_df.copy()
    df = input_df.copy()

    # Nucleons
    PDG16Plus_masses_nucleons = old_df[(old_df['Baryon no.']!=0) &\
                                 (old_df['Electric charge']==0) &\
                                 (old_df['ID']>0) &\
                                 (old_df['Isospin']==0.5) &\
                                 (old_df['Strangeness no.']==0)]['Mass(GeV)']
    ...
    PDG21_masses_nucleons = df[(df['Baryon no.']!=0) &\
                                 ...
                                 (df['Stars']>=2)]['Mass(GeV)']
    # Deltas
    PDG16Plus_masses_deltas = old_df[(old_df['Baryon no.']!=0) &\
                                 (old_df['Electric charge']==0) &\
                                 (old_df['ID']>0) &\
                                 (old_df['Isospin']==1.5) &\
                                 (old_df['Strangeness no.']==0)]['Mass(GeV)']
    ...
    PDG21_masses_deltas = df[(df['Baryon no.']!=0) &\
                                 ...
                                 (df['Stars']>=2)]['Mass(GeV)']
    # Kaons
    PDG16Plus_masses_kaons = old_df[(old_df['Baryon no.']==0) &\
                              (abs(old_df['Strangeness no.'])==1) &\
                              (old_df['Electric charge']==0) &\
                              (old_df['ID']>0)]['Mass(GeV)']
    ...
    # Lambdas
    PDG16Plus_masses_lambdas = old_df[(old_df['Baryon no.']!=0) &\
                                 (old_df['Electric charge']==0) &\
                                 (old_df['ID']>0) &\
                                 (old_df['Isospin']==0) &\
                                 (old_df['Strangeness no.']==-1)]['Mass(GeV)']
    ...
    PDG21_masses_lambdas = df[(df['Baryon no.']!=0) &\
                                 ...
                                 (df['Stars']>=2)]['Mass(GeV)']
    # Xis
    PDG16Plus_masses_xis = old_df[(old_df['Baryon no.']!=0) &\
                            (old_df['Isospin']==0.5) &\
                            (old_df['Electric charge']==-1) &\
                            (old_df['ID']>0) &\
                            (abs(old_df['Strangeness no.'])==2)]['Mass(GeV)']
    ...
    PDG21_masses_xis = df[(df['Baryon no.']!=0) &\
                            ...
                            (df['Stars']>=2)]['Mass(GeV)']
    # Sigmas
    PDG16Plus_masses_sigmas = old_df[(old_df['Baryon no.']!=0) &\
                                 (old_df['Electric charge']==0) &\
                                 (old_df['ID']>0) &\
                                 (old_df['Isospin']==1) &\
                                 (old_df['Strangeness no.']==-1)]['Mass(GeV)']
    ...
    PDG21_masses_sigmas = df[(df['Baryon no.']!=0) &\
                                 ...
                                 (df['Stars']>=2)]['Mass(GeV)']
    # Omegas
    PDG16Plus_masses_omegas = old_df[(old_df['Baryon no.']!=0) &\
                                 (old_df['Electric charge']==-1) &\
                                 (old_df['ID']>0) &\
                                 (old_df['Isospin']==0) &\
                                 (old_df['Strangeness no.']==-3)]['Mass(GeV)']
    ...
    #y-axis
    ax1.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax5.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax8.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax12.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax15.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax19.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax15.set_xlabel(r'PDG16+',fontsize=12)
    ax16.set_xlabel(r'PDG21+',fontsize=12)
    ax17.set_xlabel(r'PDG21',fontsize=12)
    ax19.set_xlabel(r'PDG16+',fontsize=12)
    ax20.set_xlabel(r'PDG21+',fontsize=12)
    ax21.set_xlabel(r'PDG21',fontsize=12)
    ...
    fname = '../hadron_lists/PDG16Plus_vs_PDG21Plus_spectrum.pdf'
...
def plot_mass_spectra_new(input_df):
    ...
    #y-axis
    ax1.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax4.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax6.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax9.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax11.set_ylabel(r'Mass (GeV)',fontsize=13)
    ax14.set_ylabel(r'Mass (GeV)',fontsize=13)
    ...
    ax1.text(0.2,1.1, r"$N$", fontsize=14)
    for mass in PDG21Plus_masses_nucleons.to_list():
        ax1.axhline(mass,xmin=eps,xmax=1-eps,linewidth=lw,color='g')
    for mass in PDG21_masses_nucleons.to_list():
        ax2.axhline(mass,xmin=eps,xmax=1-eps,linewidth=lw,color='b')
    # Deltas
    ax4.text(0.2,1.3, r"$\Delta$", fontsize=14)
    for mass in PDG21Plus_masses_deltas.to_list():
        ax4.axhline(mass,xmin=eps,xmax=1-eps,linewidth=lw,color='g')
    for mass in PDG21_masses_deltas.to_list():
        ax5.axhline(mass,xmin=eps,xmax=1-eps,linewidth=lw,color='b')
    # Sigmas
    ...
    for mass in PDG21Plus_masses_xis.to_list():
        ax11.axhline(mass,xmin=eps,xmax=1-eps,linewidth=lw,color='g')
    for mass in PDG21_masses_xis.to_list():
        ax12.axhline(mass,xmin=eps,xmax=1-eps,linewidth=lw,color='b')
    # Omegas
    ax14.text(0.2,1.1, r"$\Omega$", fontsize=14)
    for mass in PDG21Plus_masses_omegas.to_list():
        ax14.axhline(mass,xmin=eps,xmax=1-eps,linewidth=lw,color='g')
    for mass in PDG21_masses_omegas.to_list():
        ax15.axhline(mass,xmin=eps,xmax=1-eps,linewidth=lw,color='b')

    plt.tight_layout(h_pad=-0.1,w_pad=-3.7)
    fname = '../hadron_lists/PDG21Plus_spectrum.pdf'
    plt.savefig(fname=fname, format='pdf')
    #plt.show()
